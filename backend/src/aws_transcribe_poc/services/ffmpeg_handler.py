...
class FfmpegHandler:
    ...
    def convert_to_wav(self, input_file) -> tuple[str, dict]:
        ...
        try:
            ...
            logger.info(
                f"Converting {input_file} ({input_size_mb:.2f} MB) to WAV format"
            )
            ...
            metrics = {
                ...
                "size_reduction_percent": round(
                    (1 - output_size_bytes / input_size_bytes) * 100, 2
                )
            ...
            logger.info(
                f"Conversion complete: {output_name} ({convert_duration:.2f}s, {output_size_mb:.2f} MB)"
            )
            return output_name, metrics

        except Exception as e:
            logger.error(f"Ffmpeg conversion failed - {input_file}: {e!s}")
            raise RuntimeError(f"FFmpeg conversion failed for {input_file}: {e!s}")
