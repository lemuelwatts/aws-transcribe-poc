    ...
    def generate_embedding(self, audio_path: str) -> np.ndarray:
        ...
        try:
            ...
            embedding = self.model.encode_batch(
                signal
            )  # returns Torch Tensor, shape [batch_size, embedding_dim]
    ...
    def find_match(
        ...
        try:
            ...
            for speaker_name, stored_embedding in stored_embeddings.items():
                ...
                logger.debug(
                    f"Comparing with '{speaker_name}': similarity={similarity:.3f}"
                )
            ...
            if best_similarity >= threshold:
                logger.info(
                    f"Match found: '{best_match_name}' (similarity: {best_similarity:.3f})"
                )
                return best_match_name, best_similarity
            else:
                logger.info(
                    f"No match above threshold {threshold}. Best was {best_similarity:.3f}"
                )
