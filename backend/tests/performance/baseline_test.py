...
def process_file(file_path: Path) -> dict[str, Any]:
    ...
    with open(file_path, "rb") as f:
        ...
        response = requests.post(
            f"{API_BASE_URL}/audio/process", files=files, timeout=1800
        )
...
def transcribe_file(s3_uri: str) -> dict[str, Any]:
    ...
    response = requests.post(
        f"{API_BASE_URL}/transcribe", json={"s3_uris": [s3_uri]}, timeout=3600
    )
...
def run_pipeline(file_path: str) -> dict[str, Any]:
    ...
    audio_metrics = audio_result["metrics"].copy
    audio_metrics.pop(
        "input_metadata", None
    )  # remove the input_metadata to clean up long metrics
...
def generate_summary(results: list[dict[str, Any]]) -> None:
    ...
    avg_ffmpeg = sum(
        r["audio_processing"]["ffmpeg_conversion_time_seconds"] for r in results
    ) / len(results)
    avg_s3 = sum(
        r["audio_processing"]["s3_upload_time_seconds"] for r in results
    ) / len(results)
